import os, io, json, tempfile, time, csv, datetime, threading, math, traceback
...
CLIP_SECONDS = int(os.getenv("CLIP_SECONDS", "30"))  # preview seconds
QUERY_SECONDS = int(os.getenv("QUERY_SECONDS", "90"))# user track seconds
...
# --- Lazy ffmpeg path for pydub ---
def ensure_ffmpeg():
    try:
        # honor env if provided
        ff = os.getenv("FFMPEG_BINARY")
        if ff and os.path.isfile(ff):
            AudioSegment.converter = ff
            return ff
        # otherwise try imageio-ffmpeg lazily (download if needed)
        import imageio_ffmpeg
        path = imageio_ffmpeg.get_ffmpeg_exe()
        AudioSegment.converter = path
        return path
    except Exception as e:
        # leave converter unset; decoding may fail later with clear message
        return None
...
# --- Spectral fingerprint (constellation-like) ---
...
def find_peaks_per_frame(spec: np.ndarray, n_peaks: int):
    peaks = []
...
def hash_pairs(peaks, fanout, dt_min, dt_max):
    pairs = []
    for t1, freqs in enumerate(peaks):
        for f1 in freqs[:fanout]:
...
def mono_resample(raw: bytes, ext: str, sr: int, max_seconds: int) -> np.ndarray:
    ...
    ff = ensure_ffmpeg()
    if not ff:
        raise RuntimeError("FFmpeg pole saadaval. Sea env FFMPEG_BINARY või lubada imageio-ffmpeg allalaadimine.")
...
# --- iTunes search ---
def itunes_search(term: str, limit: int = 25):
    url = "https://itunes.apple.com/search"
    r = requests.get(url, params={"term": term, "media": "music", "limit": limit}, timeout=20)
    r.raise_for_status()
...
def autocatalog(queries: str = Form("james brown, aretha franklin, daft punk, funkadelic, the winstons"),
    ...
    for q in [x.strip() for x in (queries or "").split(",") if x.strip()]:
        try:
            ...
            log_exc("itunes_search failed")
            continue
        for r in results:
            tid = str(r.get("trackId") or r.get("collectionId") or r.get("previewUrl") or r.get("trackViewUrl"))
...
async def scan_json(request: Request, file: UploadFile = File(...), start_seconds: int = Form(0)):
    ...
    ranked.sort(key=lambda x: x["votes"], reverse=True)
    top = [r for r in ranked if r["votes"] >= MATCH_VOTE_THRESHOLD][:50]
    return {"pairs_query": len(pairs), "matches": matches, "threshold": MATCH_VOTE_THRESHOLD, "results": top}
